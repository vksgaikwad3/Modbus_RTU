...
void constructPacket()				// Packet gets construced here (FRAME of packet)
{	 
  packet->requests++;
  frame[0] = packet->id;
...
void processReply()    // when everything goes correct then slave will response as below
{
	// combine the crc Low & High bytes
  unsigned int received_crc = ((frame[buffer - 2] << 8) | frame[buffer - 1]);  //last 2 bytes of CRC
...
void modbus_configure(HardwareSerial* SerialPort,
											long baud,
											unsigned char byteFormat,      //Serial byte format like 8N2,8N1 etc.
											long _timeout, 
											long _polling, 
...
void modbus_construct(Packet *_packet, 											// needs this to read/write to slaves on Modbus.
											unsigned char id, 					//Slave ID
											unsigned char function,	 			//Function Code
											unsigned int address, 				// Address to read/write 
											unsigned int data,
...
void sendPacket(unsigned char bufferSize)
{
	digitalWrite(TxEnablePin, HIGH);			// enable MAX485 TxEnablePin
		
	for (unsigned char i = 0; i < bufferSize; i++)
		(*ModbusPort).write(frame[i]);					//Send packet on Serial port(ModbusPort)	
		
	(*ModbusPort).flush();				
	
	delayMicroseconds(frameDelay);
	
	digitalWrite(TxEnablePin, LOW);				// disable MAX485 TxEnablePin
